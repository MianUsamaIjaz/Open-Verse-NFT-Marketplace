...
function Header() {
  ...
  async function getNFTs() {
    const userNFTIds = await open_verse.getOwnedNFTs(CURRENT_USER_ID);
    console.log(userNFTIds);
    setOwnedGallery(<Gallery title="My NFTs" ids={userNFTIds} role="collection" />);

    const listedNFTIds = await open_verse.getListedNFTs();
    ...
    <div className="app-root-1">
      <header className="Paper-root AppBar-root AppBar-positionStatic AppBar-colorPrimary Paper-elevation4">
        <div className="Toolbar-root Toolbar-regular header-appBar-13 Toolbar-gutters">
          ...
          <Link to="/">
            <h5 className="Typography-root header-logo-text">Open Verse</h5>
          </Link>
